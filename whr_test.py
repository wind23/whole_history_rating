...
class WholeHistoryRatingTest:
    ...
    def test_winrates_should_be_inversely_proportional_with_unequal_ranks(self):
        game = self.setup_game_with_elo(600, 500, 0)
        assert (game.white_win_probability() - (1-game.black_win_probability()) < 0.0001)
    ...
    def test_output(self):
        ...
        assert ([[1, -92, 71], [2, -94, 71], [3, -95, 71], [4, -96, 72]] == list(map(lambda x: list(map(round, x)), self.whr.ratings_for_player("shusaku"))))
        assert ([[1, 92, 71], [2, 94, 71], [3, 95, 71], [4, 96, 72]] == list(map(lambda x: list(map(round, x)), self.whr.ratings_for_player("shusai"))))
